...
### Portal design, P1 — 19 September
From [design/portal-design-plan.md](../design/portal-design-plan.md), after Andreas's decisions (sidebar, footer stays, both numbers and people on the board page, Danish first). Landed: the sidebar shell with role groups and count badges (`PortalShell`, `lib/portal.ts`), the page header pattern (`PageHeader`) on every signed-in page, the status chip (`StatusChip`), the front page as two dashboards (specialist: profile state and checklist, requests, next meeting; board: queues with age, who is waiting, response time, seat map, activity), and Møder as its own page (`/portal/moeder`). Next: P2 (queues as master–detail, notes timeline, toast and confirm, empty states).
