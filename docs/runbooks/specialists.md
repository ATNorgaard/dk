...
**CV import.** Upload a PDF or paste LinkedIn text; `/api/cv-import` sends it through OpenRouter (OpenAI-style chat completions, plain `fetch`, no SDK) with a structured output schema in the profile's shape and returns a proposal, validated with Zod before it reaches the browser. The specialist reads it and clicks "Brug forslaget"; `applyImport` fills only empty fields and appends CV rows, never overwriting. Needs `OPENROUTER_API_KEY` on the server (Vercel and `.env.local`); without it the section says it is not enabled. The model is `OPENROUTER_MODEL`, default `deepseek/deepseek-v4.1-flash` (Andreas's choice, 17 September); any OpenRouter id with structured outputs works, so switching provider is one env var. A CV is a few thousand tokens, so cost per import is a fraction of a cent on Flash. Reasoning is switched off in the request (`reasoning: { enabled: false }`): with it on, DeepSeek spent the whole output budget thinking and returned empty content or hit the 110 s timeout; with it off a CV takes 35 to 45 s (17 September). The route's `maxDuration` is 120 s, which needs Fluid compute on Vercel (the project's default). Language names the model returns ("dansk") are normalised to ISO codes on the server. PDFs go through OpenRouter's file parser first (`OPENROUTER_PDF_ENGINE`, default `pdf-text`, free); a scanned PDF with no text layer needs `mistral-ocr`, and a model that reads PDFs itself can use `native`.
