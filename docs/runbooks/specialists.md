...
**CV import.** Upload a PDF or paste LinkedIn text; `/api/cv-import` sends it to Claude with a structured output schema in the profile's shape and returns a proposal. The specialist reads it and clicks "Brug forslaget"; `applyImport` fills only empty fields and appends CV rows, never overwriting. Needs `ANTHROPIC_API_KEY` on the server (Vercel and `.env.local`); without it the section says it is not enabled. Model `claude-opus-5`; a CV is a few thousand tokens, so cost per import is cents.
