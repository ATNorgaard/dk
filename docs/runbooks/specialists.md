...
`/[lang]/portal/min-side`, visible in the portal nav for anyone with the `specialist` role. Sections, in order: CV import first (so a new specialist starts by importing, then corrects the fields it filled), basics (title, one-line tagline, city, years), portrait (uploaded straight from the browser into the `portraits` bucket, under the person's own folder), about (summary, skills one per line, languages as two-letter codes), availability and rate (rate is only ever shown to clients), links (LinkedIn, website, the slug in the address), experience, education, certifications, publishing. Every text field is Danish and English; an empty English field falls back to the Danish. The status line says draft / published but seat not active / live, and a rough completeness percentage.

**CV import.** Upload a PDF or paste LinkedIn text; `/api/cv-import` sends it through OpenRouter (OpenAI-style chat completions, plain `fetch`, no SDK) with a structured output schema in the profile's shape and returns a proposal, validated with Zod before it reaches the browser. The specialist reads it and clicks "Brug forslaget"; `applyImport` fills only empty fields and appends CV rows, never overwriting. Needs `OPENROUTER_API_KEY` on the server (Vercel and `.env.local`); without it the section says it is not enabled. The model is `OPENROUTER_MODEL`, default `google/gemini-3.1-flash-lite` (Andreas's choice, 17 September); any OpenRouter id with structured outputs works, so switching provider is one env var. Five models were run on the same test CV that day, text and PDF, same prompt and schema: Gemini 3.1 Flash-Lite 3.5 to 4 s and about $0.002 per import, faithful and fluent if a little generic; GPT-5.4 mini 4 to 6 s, $0.004, the most concise; Claude Sonnet 5 9 to 11 s, $0.016, the best Danish prose; Claude Haiku 4.5 6 to 16 s, embellished beyond the source; DeepSeek V4.1 Flash 11 to 34 s depending on which provider OpenRouter picked, weakest text (copied the CV's first person into the summary). The section opens with a notice that the proposal is machine-written and must be read and corrected before use. Reasoning is switched off in the request (`reasoning: { enabled: false }`): with it on, DeepSeek spent the whole output budget thinking and returned empty content or hit the 110 s timeout. The route's `maxDuration` is 120 s, which needs Fluid compute on Vercel (the project's default). Language names the model returns ("dansk") are normalised to ISO codes on the server. PDFs reach the model through OpenRouter's file parser (`OPENROUTER_PDF_ENGINE`, default `native`: Gemini reads the PDF itself); `pdf-text` extracts the text first for models without file input, and a scanned PDF with no text layer needs `mistral-ocr`.
