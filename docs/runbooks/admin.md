...
- **See the site as another role ("Se som"):** admins get a small selector right of the header logo on every page: Admin (mig), Bestyrelse, Specialist, Kunde, Besøgende. It sets a cookie (`tuc-view-as`, one day) that `getViewer` applies, so pages, nav and gates behave as for that role; "Besøgende" makes the portal and admin pages send you to the login page, and the selector stays put so you can switch back. It changes what the pages think you are, not what the database lets your session read: row-level security still sees the admin, so a list filtered only by policy still shows everything. Server actions run under the viewed role too, so saving as "Besøgende" is refused.
