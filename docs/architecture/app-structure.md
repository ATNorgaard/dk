...
- Specialists (phase 2.3): `[lang]/portal/min-side/page.tsx` (the editor), `[lang]/specialister/[slug]/page.tsx` (public teaser, gated full CV), `src/app/api/cv-import/route.ts` (OpenRouter, structured output, Zod-validated), `src/lib/specialists.ts`, `src/app/actions/profile.ts`, `src/components/profile/`, `src/content/specialists.ts`. `HouseStage` takes a `teasers` prop from the landing page for level two. See [runbooks/specialists.md](../runbooks/specialists.md).
- Client access (phase 2.4): `src/components/forms/AccessRequestForm.tsx` behind the gate on the specialist page, `submitAccessRequest` in `actions/intake.ts`, `[lang]/admin/adgang/page.tsx` with `decideAccess` in `actions/admin.ts`, mails `accessRequestReceived`, `accessApproved`, `accessDeclined`, `newAccessRequestNotice`.
- Bookings (phase 2.5): `src/components/booking/{BookingForm,BookingInbox}.tsx`, `[lang]/booking/[id]/page.tsx` (token page), `[lang]/admin/bookinger/page.tsx`, `src/lib/bookings.ts`, `src/app/actions/bookings.ts`, `src/lib/email/ics.ts`, `src/lib/supabase/admin.ts` (service client, server-only, token-checked paths only), `src/content/bookings.ts`.
